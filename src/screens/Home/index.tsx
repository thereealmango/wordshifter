...
const HomeScreen: React.FC = () => {
  ...
  const [showSettingsId, setShowSettingsId] = useState<number | undefined>(
    undefined
  );
  ...
  const renderBlurredBGSettings = () => {
    ...
    return (
      <>
        ...
        <Pressable style={styles.closeSettings} onPress={closeSettings} />
      </>
    );
  };
  ...
  const renderFolders = () => {
    return (
      <View
        style={
          isNumber(showSettingsId)
            ? styles.listConteinerAbsolute
      ...
      >
        ...
        {folders?.map((item) => (
          <Pressable
            key={item.id}
            style={
              showSettingsId == item.id ? styles.folder : styles.focusedFolder
            }
      ...
      <Button
        ...
        text={
          selectedFolders.length > 0
            ? t.home.removeSelectedFolders
            : t.home.createSetButton
        }
        onPress={
          selectedFolders.length > 0 ? removeSelectedFolders : openAddModal
        }
      />
    </View>
  );
